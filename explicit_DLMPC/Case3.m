...
%                 Phi_loc{i} = result.x(:);
                ...
                criterion = rho*a*ADMM_matrix*xi_i;

                if criterion-b1>0

                    lamb1 = (criterion-b1)/(xi_i'*ADMM_matrix*xi_i);

                elseif -criterion+b2>0

                    lamb2 = (-criterion+b2)/(xi_i'*ADMM_matrix*xi_i);
                end

                Phi_loc{i} = (rho*a-(lamb1-lamb2)*xi_i')*ADMM_matrix;
...
%                     Phi_loc{i} = result.x(:);
                ...
                Phi_loc{i} = (rho*a-(lambda(1)-lambda(2))*xi_i')*ADMM_matrix;
...
obj(index)=0;
for t =1:Tsim
obj(index) = obj(index) + x(:,t)'*Q*x(:,t)+u(:,t)'*S*u(:,t);
end
obj(index) = obj(index) + x(:,t+1)'*Q*x(:,t+1);


obj_VAL(index)=0;
for t =1:Tsim
obj_VAL(index) = obj_VAL(index) + x_VAL(:,t)'*Q*x_VAL(:,t)+u_VAL(:,t)'*S*u_VAL(:,t);
end
obj_VAL(index) = obj_VAL(index) + x_VAL(:,t+1)'*Q*x_VAL(:,t+1);

end

% Save to .txt
index = 0;
fid=fopen('Optimal_cost_validation.txt','w');
header1 = 'Distributed MPC';
header2 = 'Centralized MPC';
fprintf(fid, [ header1 ' ' header2 'r\n']);
for network = cases
    index = index+1;
    fprintf(fid, '%f %f r\n', [obj(index) obj_VAL(index)]');
end
