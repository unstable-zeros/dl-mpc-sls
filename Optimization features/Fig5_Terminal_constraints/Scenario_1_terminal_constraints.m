...
E1 = [E1;zeros(Nx,Nx)]; % If we want to impose terminal constraint
...
IZA_ZB = [IZA_ZB; zeros(Nx,Nx*(T-1)) eye(Nx) zeros(Nx,Nu*(T-1))]; % If we want to impose terminal constraint
...
%         time(index) = time(index) + toc;
        ...
        i = 1; % Separate the first column since we want to measure time
        IZA_ZB_loc = IZA_ZB(:,s_c{i}); row_all_zeros = find(all(IZA_ZB_loc == 0,2)); keep_indices = setdiff(linspace(1,Nx*(T+1),Nx*(T+1)),row_all_zeros);
...
%         cvx_begin
% 
%         variable Psi(Nx*T+Nu*(T-1),Nx)
%         
%         vector = vec(Phi-Psi+Lambda);
%         minimize(vector'*vector)
%         IZA_ZB*Psi == E1
% 
%         cvx_end
...
for k = 1:Tsim
    ...
    % Simulate what the dynamics are given that action
    x_MPC(:,k+1) = x_aux(:,2); % Since there is no noise x_ref = x
...
obj(iteration)=0;
for t =1:Tsim
obj(iteration) = obj(iteration) + x(:,t)'*Q*x(:,t)+u(:,t)'*S*u(:,t);
end
obj(iteration) = obj(iteration) + x(:,t+1)'*Q*x(:,t+1);


obj_VAL(iteration)=0;
for t =1:Tsim
obj_VAL(iteration) = obj_VAL(iteration) + x_VAL(:,t)'*Q*x_VAL(:,t)+u_VAL(:,t)'*S*u_VAL(:,t);
end
obj_VAL(iteration) = obj_VAL(iteration) + x_VAL(:,t+1)'*Q*x_VAL(:,t+1);

obj(iteration)-obj_VAL(iteration)

obj_MPC(iteration)=0;
for t =1:Tsim
obj_MPC(iteration) = obj_MPC(iteration) + x_MPC(:,t)'*Q*x_MPC(:,t)+u_MPC(:,t)'*S*u_MPC(:,t);
end
obj_MPC(iteration) = obj_MPC(iteration) + x_MPC(:,t+1)'*Q*x_MPC(:,t+1);
...
hold on
plot(1:Tsim+1,x_MPC(1,:),'-r',1:Tsim+1,x_MPC(3,:),'m')
