...
data_rvl_rob_offline = cell(length(ds), 1); 
...
for i = 1:length(ds)
    ...
    data_rvl_rob_offline{i} = [tStats.time_, tStats.iters_];
end

name = strcat('rvl_rob_off_seed',num2str(seed),'_ct',num2str(connectThresh*100),'.mat');
save(name);

%% Plot (not formatted for paper)
rvls = zeros(length(ds), 1);
for i = 1:length(ds)
    rvls(i) = data_rvl_rob_offline{i}(1);    
end
...
xlabel('d');
title('Runtime terminal set per state, polytopic');